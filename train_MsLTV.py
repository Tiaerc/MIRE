...
from modeling.multi_view_model import MultiSModel
...
def train(jf_game_id, train_start_date, train_end_date, val_start_date, val_end_date, save_path, features, model='DNN', device='cuda:0', max_row=None, minus_label_col=None):
    ...
    # model
    model = MultiSModel(features=features,
                           hidden_unit=[128],
                           encode_dim=64,
...
if __name__ == '__main__':
    ...
    for jf_game_id in ['mdnf']:
        # save_path = os.path.join(get_root_path(), 'checkpoints/mdnf', f'{jf_game_id}_MsLTV_6h_rec{args.rec_loss_weight}_clus{args.cl_loss_weight_us}_kl{args.kl_weight}')
        save_path = os.path.join(get_root_path(), 'checkpoints/mdnf', f'{jf_game_id}_test_rec{args.rec_loss_weight}_clus{args.cl_loss_weight_us}_kl{args.kl_weight}')
